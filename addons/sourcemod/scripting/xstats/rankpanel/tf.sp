...
stock void DBQuery_TotalStatistics_TF2(Database database, DBResultSet results, const char[] error, int client)	{
	...
	TotalStats[client].DeflectKills = results.FetchInt(14);
	TotalStats[client].GibKills = results.FetchInt(15);
...
void RankPanel_Total_TF2(int client, int page)	{
	...
	switch(page)	{
		case 1:	{
			...
			panel.DrawText("%i Deflect kills", TotalStats[client].DeflectKills);
			panel.DrawText("%i Gib kills", TotalStats[client].GibKills);
			panel.DrawText("%i Crit kills", TotalStats[client].CritKills);
