...
stock void Player_Death_CSS(Event event, const char[] event_name, bool dontBroadcast)	{
	...
	Format(query, sizeof(query), "update `%s` set Kills = Kills+1 where SteamID='%s'", Global.playerlist, Player[client].SteamID);
	DB.Threaded.Query(DBQuery_Callback, query);
	
	Format(query, sizeof(query), "update `%s` set Kills_%s = Kills_%s+1 where SteamID='%s'", Global.playerlist, weapon, weapon, Player[client].SteamID);
	DB.Threaded.Query(DBQuery_Callback, query);
	...
	if(points > 0)	{
		...
		Format(query, sizeof(query), "update `%s` set Points = Points+%i", Global.playerlist, points);
		DB.Threaded.Query(DBQuery_Callback, query);
