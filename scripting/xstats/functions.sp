...
stock int GetClientPlayTime(const char[] auth)
{
	...
	if(database != null)
	{
		DBResultSet results = SQL_QueryEx(database, "select PlayTime from `%s` where SteamID='%s'", playerlist, auth);
 ...
 */
stock void DBQuery_Death(Database database, DBResultSet results, const char[] error, any data)
{
	if(results == null)
	{
